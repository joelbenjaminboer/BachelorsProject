...
class IMUPreprocessor:
    ...
    def run(self, output_dir):
        ...
        for i, test_subj in enumerate(valid_subject_ids):
            ...
            save_fold_to_hdf5(os.path.join(fold_dir, "data.h5"), X_train, y_train, X_val, y_val, X_test, y_test)
...
def run_preprocessing(cfg: DictConfig, version: str = "0.1.0"):
    ...
    preprocessor = IMUPreprocessor(
        ...
        horizon=horizon,
        original_freq=500,
        target_freq=cfg.dataset.get("target_freq", 100) # Ensure 100Hz based on originally planned 100Hz
    )
